...
import '../services/image_detector_service.dart';

class ImageDetectorScreen extends StatefulWidget {
  const ImageDetectorScreen({super.key});

  @override
  State<ImageDetectorScreen> createState() => _ImageDetectorScreenState();
}

class _ImageDetectorScreenState extends State<ImageDetectorScreen> {
  final service = ImageDetectorService();
  ...
  Color get barColor => const Color(0xFFD28B00);
  Color get buttonColor => const Color(0xFFFFA200);
  ...
  Future<void> _pickImage() async {
    ...
    final detectedMime = lookupMimeType(f.name) ?? 'image/jpeg';
  ...
  Widget _inputBox() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(
            selected == null ? "Upload Image\nJPG/PNG" : "Image Selected ✅",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:  Text(AppStrings.checkImage(context)),
        centerTitle: true,
      ),
